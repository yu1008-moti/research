...
fin_summary.duckdb では，決算情報に関する情報の内サマリー情報（決算短信）を格納している．<br>
本研究における決算情報に対する処理・利用方法をまとめる．

## 特徴量に対する処理

本節では，特徴量にたいする処理や変数変換についてまとめる
...
|特徴量|運用方法|
|---|---|
|DiscDate|CASTにより，`Date`型に変換する．TimeEmbeddingが有力な使い道として想定しているが，詳細は未定である．また，データを一意に識別するためにも使用する．|
|CurPerType|当会計期間の種類であり，以下の様な値をとる：<br>- `Q1`：第一四半期<br>- `Q2`：第二四半期<br>- `Q3`：第三四半期<br>- `FY`：期末<br>以上のデータを，埋め込み・インクリメント等の方法で処理する必要があるが，感覚的には後者でいいと思われる．ここは，今後の実験で得られる結果から試行錯誤するべきである．|
|CurPerSt|当会計期間開始日．TimeEmbeddingが有力な使い道として想定しているが，詳細は未定である．|
|CurPerEn|当会計期間終了日．TimeEmbeddingが有力な使い道として想定しているが，詳細は未定である．|

また，`CurPerType`はもう何種類かあるのだが，重要度は低いと考え除去した．<br>
以下のクエリによって絞る

```sql
CurPerType NOT IN ('4Q', '5Q') AND CurPerType IS NOT NULL
```

さらに，DocTypeについては以下のクエリによって絞る

```sql
REGEXP_MATCHES(DocType, '([1-4]Q|FY)FinancialStatements_Consolidated_JP')
```

これによって，以下の書類にのみ対象とする
...
配当情報としてDiv系が存在し，次のように処理する．

|特徴量|運用方法|
|---|---|
|Div|一株あたり配当実績_当期末期末|
|DivAnn|一株あたり配当実績_合計|
|FDivNxQ|一株あたり配当予想_次期期末|

まず，`Div1Q`～`DivFY`の特徴量は，`CurPerType`に応じて該当する配当特徴量を選択する．続いて，`DivAnn`がNULLの場合は，`Div1Q`～`DivFY`の合計値を用いる．さらに，`FDiv1Q`～`FDivFY`の特徴量は，`FDiv1Q`～`FDivFY`がNULLの場合は，`NxFDiv1Q`～`NxFDivFY`を用いる．

以上の処理をまとめると，以下のSQLのようになる．
...
ただし，効果があるのかどうかはよくわからない．そのため，実験で効果を確かめながら，適宜調整する必要があると考えている．**最終的に，使わない可能性すらある**．感覚的に重要な気がするため，とりあえずは使うことを前提にしている．
...
|特徴量|運用方法|
|---|---|
|Sales・FSales|売上高・予想売上高が格納されている．<br>年度末に向かい積み上げられていくデータであり，当期の売上高を得るには差分を取る必要がある．|
|OprP・FOprP|営業利益・予想営業利益が格納されている．|
|OrdP・FOrdP|経常収益・予想経常収益が格納されている．|
|NetP・FNetP|純利益・予想純利益が格納されている．|
|CFO・CFI・CFF・CashEq|キャッシュフロー|
|TA・Eq・EqAR|資産・負債・株主資本|
|IsSh・TrSh|株式数|
|EPS・FEPS・BPS|1株当たり利益|

### 処理詳細

SQLの処理を追うことで，特徴量の処理を詳細に解説する．
...
CREATE OR REPLACE TEMP MACRO getLag(x, w, Code, CurFYEn, CurPerType, DiscDate) AS(
    CASE
        WHEN w = 'Y' THEN LAG(x) OVER (
            PARTITION BY Code, CurFYEn    
            ORDER BY DiscDate 
            ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
        )
        WHEN w = 'Q' THEN LAG(x) OVER (
            PARTITION BY Code, CurPerType 
            ORDER BY DiscDate 
            ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
        )
        WHEN w = 'A' THEN LAG(x) OVER (
            PARTITION BY Code
            ORDER BY DiscDate 
            ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
        )
        WHEN w = 'S' THEN LAG(x) OVER (
            PARTITION BY Code
            ORDER BY DiscDate
        )
    END
...
|マクロ名|説明|
|-|-|
|`Cast2Date`|文字列型の日時を`Date`型に変換する．後に，日付同士で比較できるようにすることで，日付の計算が可能になるだけでなく，`DATETIME`型よりもメモリ使用量が少ない．|
|`isExist`|値が`NULL`かどうかを判定する．`NULL`の場合は0，それ以外は`1`を返す．|
|`CalcRatio`|分子と分母の両方が`0`の場合は`0`を返し，それ以外は分子/分母を返す．|
|`ffill`|前方埋めを行う．引数`w`に応じて，前方埋めの範囲を変えることができる．|
|`getLag`|前方埋めの値を取得する．引数`w`に応じて，前方埋めの範囲を変えることができる．|
|`myLog`|値が正の場合は`log`を返し，負の場合は`-log`を返す．0の場合は0を返す．|
...
|CTE名|説明|
|-|-|
|ini|全体の初期化を担う．元のテーブルである`fin_sum`から全ての特徴量を抽出し，以下の特徴量にはそれぞれ処理を実施する．<br>- `DiscDate` : 開示日であり，`Date`型に変換される．<br>- `CurFYEn` : 会計年度終了日であり，`Date`型に変換される．<br>- `CurPerSt` : 会計期間開始日であり，`Date`型に変換される．<br>- `CurPerEn` : 会計期間終了日であり，`Date`型に変換される．<br>また，新たに特徴量を設定するが，WHERE句で行を絞るために使用するためだけに使用する．<br>- `L_DiscDate` : その`Code`における前回の開示日を示す．<br>- `gap_DiscDate_CurPerEn` : その`Code`における前回の開示日から当期の会計期間終了日までの期間．<br>WHERE句では，以下の二つの条件を指定する．<br>1. `CurPerType` が `1Q`, `2Q`, `3Q`, `FY` だけを扱う．<br>2. 正規表現`'([1-3]Q\|FY)FinancialStatements_Consolidated_JP'`と合致する`DocType`だけ扱う．|
|base||
|EasyRepl||
|isExistBinary||
|ForwardFill_pre||
...
ini AS(
    ...
    WHERE
        ...
        REGEXP_MATCHES(DocType, '([1-3]Q|FY)FinancialStatements_Consolidated_JP')
...
```sql
    ForwardFill AS(
        SELECT 
            b.DiscDate, b.Code, b.CurFYEn, b.CurPerType, 
            ...
            ffill(
                COALESCE(b.EqAR, CalcRatio(fp.Eq_tmp, fp.TA_tmp), 0), 
                'A', b.Code, b.CurFYEn, b.CurPerType, b.DiscDate
            ) AS EqAR_tmp,
...
```sql
    ReallyGain AS(
        SELECT 
            ...
            f.Sales_tmp - COALESCE(
                getLag(f.Sales_tmp, 'Y', b.Code, b.CurFYEn, b.CurPerType, b.DiscDate), 0
            ) AS Sales_ReallyGain_tmp,

            f.OP_tmp    - COALESCE(
                getLag(f.OP_tmp,    'Y', b.Code, b.CurFYEn, b.CurPerType, b.DiscDate), 0
            ) AS OP_ReallyGain_tmp,

            f.OdP_tmp   - COALESCE(
                getLag(f.OdP_tmp,   'Y', b.Code, b.CurFYEn, b.CurPerType, b.DiscDate), 0
            ) AS OdP_ReallyGain_tmp,
            
            f.NP_tmp    - COALESCE(
                getLag(f.NP_tmp,    'Y', b.Code, b.CurFYEn, b.CurPerType, b.DiscDate), 0
            ) AS NP_ReallyGain_tmp,
        FROM base b
        JOIN ForwardFill f
            USING(DiscDate, Code, CurFYEn, CurPerType)
    ),
...
```sql
    PreQ_Vals AS(
        SELECT 
            ...
            COALESCE(
                LAST_VALUE(b.FDiv1Q IGNORE NULLS) OVER (
                    PARTITION BY b.Code, b.CurPerType 
                    ORDER BY b.DiscDate 
                    ROWS BETWEEN 1 PRECEDING AND CURRENT ROW
                ), 0
            ) AS FDiv1Q_tmp,
            COALESCE(
                LAST_VALUE(b.FDiv2Q IGNORE NULLS) OVER (
                    PARTITION BY b.Code, b.CurPerType 
                    ORDER BY b.DiscDate 
                    ROWS BETWEEN 1 PRECEDING AND CURRENT ROW
                ), 0
            ...
            COALESCE(
                LAST_VALUE(b.FDiv3Q IGNORE NULLS) OVER (
                    PARTITION BY b.Code, b.CurPerType 
                    ORDER BY b.DiscDate 
                    ROWS BETWEEN 1 PRECEDING AND CURRENT ROW
                ), 0
            ...
            COALESCE(
                LAST_VALUE(b.FDivFY IGNORE NULLS) OVER (
                    PARTITION BY b.Code, b.CurPerType 
                    ORDER BY b.DiscDate 
                    ROWS BETWEEN 1 PRECEDING AND CURRENT ROW
                ), 0
...
```sql
    calculated AS(
        SELECT 
            ...
            -- 成長率
            CalcRatio(
                g.Sales_ReallyGain_tmp - p.Sales_ReallyGain_preQ_tmp, 
                ABS(p.Sales_ReallyGain_preQ_tmp)
            ) AS Sales_ReallyGain_VSpreQ_GrowthRatio,
            CalcRatio(
                g.OP_ReallyGain_tmp - p.OP_ReallyGain_preQ_tmp, 
                ABS(p.OP_ReallyGain_preQ_tmp)
            ) AS OP_ReallyGain_VSpreQ_GrowthRatio,
            CalcRatio(
                g.OdP_ReallyGain_tmp -   p.OdP_ReallyGain_preQ_tmp, 
                ABS(  p.OdP_ReallyGain_preQ_tmp)
            ) AS   OdP_ReallyGain_VSpreQ_GrowthRatio,
            CalcRatio(
                  g.NP_ReallyGain_tmp -    p.NP_ReallyGain_preQ_tmp, 
                ABS(   p.NP_ReallyGain_preQ_tmp)
            ) AS    NP_ReallyGain_VSpreQ_GrowthRatio,
 
            -- 昨年同期比
            CalcRatio(
                g.Sales_ReallyGain_tmp - p.Sales_ReallyGain_lysQ_tmp, 
                ABS(p.Sales_ReallyGain_lysQ_tmp)
            ) AS Sales_ReallyGain_VSlysQ_GrowthRatio,
            CalcRatio(
                  g.OP_ReallyGain_tmp - p.OP_ReallyGain_lysQ_tmp, 
                ABS(   p.OP_ReallyGain_lysQ_tmp)
            ) AS    OP_ReallyGain_VSlysQ_GrowthRatio,
            CalcRatio(
                g.OdP_ReallyGain_tmp - p.OdP_ReallyGain_lysQ_tmp, 
                ABS(  p.OdP_ReallyGain_lysQ_tmp)
            ) AS   OdP_ReallyGain_VSlysQ_GrowthRatio,
            CalcRatio(
                g.NP_ReallyGain_tmp - p.NP_ReallyGain_lysQ_tmp, 
                ABS(   p.NP_ReallyGain_lysQ_tmp)
            ) AS    NP_ReallyGain_VSlysQ_GrowthRatio,

            -- 達成率
            CalcRatio(
                f.Sales_tmp - f.FSales_tmp, 
                ABS(f.FSales_tmp)
                ) AS Sales_AchievementRatio,
            CalcRatio(
                f.OP_tmp - f.FOP_tmp, 
                ABS(f.FOP_tmp)
            ) AS OP_AchievementRatio,
            CalcRatio(
                f.OdP_tmp - f.FOdP_tmp, 
                ABS(f.FOdP_tmp)
            ) AS   OdP_AchievementRatio,
            CalcRatio(
                f.NP_tmp - f.FNP_tmp, 
                ABS(f.FNP_tmp)
            ) AS    NP_AchievementRatio,
...
```sql
    SimpleQuery AS(
        SELECT 
            ...
            CASE
                WHEN b.DivAnn IS NULL THEN COALESCE(SUM(Div) OVER (
                    PARTITION BY b.Code, b.CurFYEn 
                    ORDER BY b.DiscDate
                ), 0)
            ELSE b.DivAnn
            END AS DivAnn,
    ...
    GetNULLFlag AS(
        SELECT 
            ...
            LIST_REDUCE(
                LIST_VALUE(
                    * COLUMNS(
                        * EXCLUDE(DiscDate, Code, CurFYEn, CurPerType)
                    )
                ), (x, y) -> x + y
            ) AS all_sum
        FROM Aggr
