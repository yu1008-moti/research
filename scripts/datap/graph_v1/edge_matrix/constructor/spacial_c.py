...
from scripts.datap.graph_v1.sql import fetch
from scripts.datap.graph_v1.edge_matrix.element import NodeSet
from scripts.datap.graph_v1.edge_matrix.spacial import SpacialEdgeMatrix
from scripts.datap.graph_v1.edge_matrix.constructor.utils import u_common
...
class _utils_firm2firm:
    ...
    def _add_capm_factors_column(
        ...
        for i, code in enumerate(fetched_prices["Code"].unique()):
            ...
            if len(filtered_result) < window_size_for_capm:
            ...
            residuals, betas = _utils_firm2firm._rolling_capm_residual(stock_returns, market_returns, window=window_size_for_capm)
        ...
        u_common.display_note(
            ...
            content=f"{len(unmatched_codes)} is unmatched: {round(len(unmatched_codes)/codes_length, 2)}%"
        )

        u_common.display_note(
            note_title="Numbers of Available Codes",
            content=f"{len(fetched_prices['Code'].unique())} is available: {round((codes_length - len(unmatched_codes))/codes_length, 2)}%"
        )
    ...
    def _iter_prices_adjacent_matrix(
        ...
        """価格データの残差を用いて、ローリングウィンドウごとに相関行列を計算し、疎行列として返すジェネレーター関数
        Args:
            returns_residual_matrix (pd.DataFrame): 価格データの残差を含むDataFrame
            window_size_for_corr (int): 相関計算用のローリングウィンドウのサイズ
            residuals_threshold (float): 相関係数の閾値
            device (str): 使用するデバイス（CPUまたはGPU）
        Yields:
                Iterator[Tuple[float, int, torch.Tensor]]: 週IDと対応する疎行列のタプルを返すジェネレーター
        """
        ...
        for t in range(window_size_for_corr, T):
            ...
            mask = torch.abs(corr) > residuals_threshold  # 閾値処理
...
class Spacial:
    """空間方向にのエッジを構築するためのクラス
    """

    def __init__(
            self, 
            threshold: float = tc.THRESHOLD,
            window_size_for_capm: int = tc.WINDOW_SIZE,
            window_size_for_corr: int = tc.WINDOW_SIZE,
            first_week_id: int = tc.FIRST_WEEK_ID
        ) -> None:
        """空間エッジを構築するためのクラスの初期化メソッド
        Args:
            threshold (float, optional): エッジを構築する際の相関係数の閾値. Defaults to tc.THRESHOLD.
            window_size_for_capm (int, optional): CAPM計算用のローリングウィンドウのサイズ. Defaults to tc.WINDOW_SIZE.
            window_size_for_corr (int, optional): 相関計算用のローリングウィンドウのサイズ. Defaults to tc.WINDOW_SIZE.
            first_week_id (int, optional): エッジ構築を開始する週ID. Defaults to tc.FIRST_WEEK_ID.
        """
        ...
        self.window_size_for_capm : int = window_size_for_capm
        self.window_size_for_corr : int = window_size_for_corr
        self.residuals_threshold : float = threshold
        self.device : str = tc.DEVICE
        self.first_week_id : int = first_week_id


    def register_firm2firm_edge(self) -> SpacialEdgeMatrix:
        """企業 - 企業の空間エッジを構築するメソッド
        Returns:
            SpacialEdgeMatrix: 空間エッジ行列
        """
        ...
        returns_residual_matrix = _utils_firm2firm._add_capm_factors_column(
            self.result_fetched_prices, 
            window_size_for_capm=self.window_size_for_capm
        )
        ...
        for procssing_ratio, week_id, sparse_firm2firm_edge_matrix in _utils_firm2firm._iter_prices_adjacent_matrix(
            returns_residual_matrix=returns_residual_matrix, 
            window_size_for_corr=self.window_size_for_corr, 
    ...
    def register_fin2firm_edge(self) -> SpacialEdgeMatrix:
        ...
        financials_exists_matrix = _utils_firm2fin._get_financials_exists_matrix(
        ...
        for procssing_ratio, week_id, sparse_fin2firm_edge_matrix in _utils_firm2fin._construct_fin_graph(
