# graph_v2 ライブラリ 関数レベル仕様書

対象ディレクトリ: `scripts/datap/graph_v2/`

このドキュメントは以下5ファイルに定義される全関数・全メソッド・全クラスについて、
...
- [cons.py](#3-conspy) — パス・パラメータ定数の定義
- [sql.py](#4-sqlpy) — DuckDBへのfetch/insert/create処理
- [main.py](#5-mainpy) — エントリポイント（現状スタブ）
...
ノードタイプは `stock`（銘柄）と `statement`（決算情報）の2種類。
エッジタイプは以下の3つ＋自動生成される逆エッジ1つ:

| src | relation | dst | 意味 | 対称性 |
|---|---|---|---|---|
| stock | corr | stock | CAPM残差リターンのローリング相関 + 同一銘柄の週次連続エッジ | 有向（相互登録） |
| statement | prev | statement | 同一銘柄の前期決算→今期決算 | 有向 |
| statement | report | stock | 決算→対応する銘柄週次ノードへの報告 | 有向 |
| stock | rev_report | statement | 上記の自動生成逆エッジ（`REVERSE_RELATIONS` により生成） | 有向 |

node_id / edge_id の命名規則:
- ノード: `{node_type}_{week_id}_{Code}` 例: `stock_202001_1301`
...
前回指摘事項を反映した書き直し版。モジュールdocstringに記載された主な変更点:

1. train/val/testを物理ファイルで分けず、単一の HeteroData を1つだけ保存し、split はロード時にマスクで行う設計に変更（過去の「data_list上書きバグ」の構造的解消）。
2. DB構築（raw data ingestion）と `.pt` 生成（グラフ構築）を別ステップに分離（`process()` がif文の中にネストしていた問題の解消）。
3. エッジタイプのキーを `"__"` 区切りの3-tupleとして一意にパースできるよう統一。
4. ノード特徴量 `x`、`is_target` マスク、`node_id` 文字列を HeteroData に正しく格納するよう修正。文字列配列はNeighborLoaderがテンソル化できないため `.npy` に別保存。
...
| 定数 | 型 | 内容 |
|---|---|---|
| `REVERSE_RELATIONS` | `Dict[Tuple[str,str,str], str]` | 逆エッジを自動生成する関係の指定。現状 `("statement","report","stock") -> "rev_report"` のみ。対称な関係（相関エッジ等）はここに入れない。 |
| `CATEGORICAL_COLUMNS` | `Dict[str, List[str]]` | `stock`: `["S33","S17","Section_id","Mkt","Mrgn"]` / `statement`: `["CurPerType"]`。ここに列挙した列のみEmbedding用の特殊扱いになる。 |
| `DATE_COLUMNS` | `Dict[str, List[str]]` | `statement`: `["CurFYEn"]`。Time2Vec等に渡す日付列。 |
| `DEFAULT_VOCAB_DIR` | `str` | `"scripts/datap/graph_v2/DS/vocab"` |
| `TIME_ENCODING_EPOCH` | `pd.Timestamp` | `"2000-01-01"`。日付→経過日数変換の基準日。 |
...
- **設計意図**: `"<UNK>"` を用意することで、vocab構築後に出現した未知カテゴリ（例: train期間に無かった業種区分がval/testで出現）にも安全に対応できる。
...
- **返り値例**: `get_vocab_sizes(1, "stock") -> {"S33": 34, "S17": 18, ...}`
...
- **概要**: 指定 `node_type` の全ノードをDBから取得し、モデル入力に必要な各種配列に整形する。
- **処理内容**:
...
- **例外**: `src_map` / `dst_map` に存在しない node_id が来た場合、`KeyError` を再送出（メッセージに「ノード生成とエッジ生成の対象範囲がズレている可能性」を付記）。**サイレントに握りつぶさず早期検出する設計**（実装コメントで明言）。

### `class preprocess`

生の `financials_df` / `prices_df` を受け取り、`graph_node` 相当のテーブル（実体は `node` / `edge` / `feats` テーブル）へ変換・格納するクラス。

#### `__init__(self, financials_df: pd.DataFrame, prices_df: pd.DataFrame, serial_id: int = 1, vocab_dir: str = DEFAULT_VOCAB_DIR)`
- **概要**: インスタンス変数に保存するとともに、`financials_df` と `prices_df` それぞれの `week_id` ユニーク値の和集合を `self.unique_week_id`（ソート済みリスト）として保持する。
...
- **処理内容**: `financials_df["Code"]` のユニークコードごとにループし、`week_id` を `shift(1)` して前期・今期のnode_idペアを作る（`edge_weight=1.0` 固定）。
...
- **概要**: `financials_df`（→`statement`ノード）と `prices_df`（→`stock`ノード）から `node_id` / `is_target` / `ticker` / `node_type` / `time_id` を定義し、`insert.node` でDB登録する。
- **処理内容**:
  - `node_id = f'{node_type}_{week_id}_{Code}'`
  - `stock` ノードの `is_target` は `Mkt in {"0000", "0500"}` の場合に `True`（それ以外の市場区分の銘柄は予測対象外）。
  - `statement` ノードの `is_target` は常に `False`。
- **副作用**: `print` で進捗ログ。DB登録。

##### 3. カテゴリ変数のvocab構築

#### `build_category_vocabs(self) -> None`
- **概要**: `CATEGORICAL_COLUMNS` に定義された各 `(node_type, column)` について、`financials_df`/`prices_df`全期間から `build_category_vocab` でvocabを構築し、`save_vocab` でディスクに保存する。
- **設計意図（docstring）**: train/val/testを分けず全期間で構築する。これは「未来のラベルを覗き見る」リークではなく、「業種区分・市場区分としてどんな値が存在し得るか」というカテゴリ全体の定義に過ぎないため区別される、という方針が明記されている（将来の新規カテゴリに備えた `<UNK>` 枠も用意）。
- **副作用**: `print` でカテゴリ数ログ。ファイル書き込み（`save_vocab`）。

##### 4. node_feats に関する処理

#### `node_feats_define(self) -> None`
- **概要**: `stock`/`statement` 各ノードの特徴量を `{"cont": {...}, "cat": {...}, "date": {...}}` 形式のJSONとして `feats` テーブルに格納する。
- **処理内容**:
  1. `week_id`, `Code` 列の存在チェック（無ければ `ValueError`）。
  2. `cont_cols` = 全列から `week_id`, `Code`, カテゴリ列, 日付列を除いた残り。
...
- **概要**: 上記の6メソッドを決められた順序で一括実行するオーケストレーションメソッド。
- **実行順序**:
  1. `statement_prev_statement_preprocess`
  2. `statement_report_stock_preprocess`
  3. `stock_corr_stock_preprocess`
  4. `node_id_define`
  5. `build_category_vocabs`（**`node_feats_define` より前に実行し、vocabを確定させる必要があるため明示的にこの順序**）
  6. `node_feats_define`
- **副作用**: 各ステップの実行時間を `print("preprocess time", dt.now() - now)` で出力。

### `class graphDataSet(InMemoryDataset)`

全期間・全銘柄・全ノードタイプを含む「単一の」`HeteroData` を保持する PyG `InMemoryDataset`。train/val/testの分割はここでは行わない（`get_loaders` 側でマスクとして行う）。
...
- **処理内容**: `graph_info_{serial_id}.duckdb` の存在チェック。存在すれば即return。無ければ `create.graph_info(serial_id)` でテーブル作成後、`fetch.financials()` / `fetch.prices()` を取得して `preprocess` を構築し、`insert_to_graph_info_table()` を実行。
...
- **使い方（docstringのコメント例）**:
  ...
  ```
  学習ループ内でバッチに含まれるノードの実体（文字列ID）を特定する用途。

#### `_build_full_graph(self) -> HeteroData`
- **概要**: DBから全ノード・全エッジを取得し、単一の `HeteroData` を構築する中核メソッド。
- **処理内容**:
  1. **ノード側**: `["stock", "statement"]` の各 `node_type` について `fetch_node_table` を呼び、`data[node_type].x / cat_x / date_x / is_target / time_id` を設定。同時に `node_str_id` を `.npy` として保存し、`build_node_id_to_idx` で `node_id_to_idx[node_type]` 辞書を構築（エッジ側の整数化に使用）。
  2. **エッジ側**: `fetch_edge_index` / `fetch_edge_attr("edge_type")` / `fetch_edge_attr("edge_weight")` / `fetch_edge_attr("observable_time_id")` を取得。`edge_type` のユニーク値（`edge_type_names`）ごとにループし、`"__"` で `(src_t, rel, dst_t)` に分解、該当行だけを `map_edge_ids_to_idx` で整数インデックス化して `data[src_t, rel, dst_t].edge_index / edge_weight / edge_time` に格納。
...
- **副作用**: `processed_dir` へのディレクトリ作成、`node_str_id_*.npy` の書き込み。
...
- **処理内容**:
  1. `graphDataSet(root="scripts/datap/graph_v2/DS", serial_id=serial_id)` を構築し、`data = dataset[0]` を取得。
  ...
  4. `num_neighbors` が未指定の場合、以下のデフォルトを使用（4つの関係それぞれ2-hop、各hop10近傍）:
     ```python
     {
...
- **設計意図**: グラフ自体は分割しない（過去情報へのアクセスを維持するため）。`is_target` かつ該当期間の `stock` ノードのみを `input_nodes`（＝バッチ生成の起点かつ損失計算対象）とすることで、`statement` ノードや非対象銘柄がバッチの起点にならないようにしている。

### 関数: `mock_code(serial_id: int = 1)`
- **概要**: `get_loaders` の疎通確認用モックコード。3つのローダーそれぞれを全件イテレートし、`batch` を `print` するだけ。
- **用途**: パイプライン全体（DB構築〜HeteroData構築〜NeighborLoader）が例外なく動作するかの動作確認。
...
CAPM残差相関エッジ構築のハイパーパラメータ（graph_v1の `train_constants` 相当）。

| 定数 | 値 | 意味 |
|---|---|---|
| `RESIDUAL_STORED_PATH` | `"./scripts/datap/graph_v2/store/returns_residual_matrix.parquet"` | CAPM残差行列のキャッシュ先 |
| `WINDOW_SIZE_FOR_CAPM` | `52` | CAPMをローリング推定する窓（週） |
| `WINDOW_SIZE_FOR_CORR` | `52` | 残差リターンのローリング相関を取る窓（週） |
| `THRESHOLD` | `0.7` | `|corr| > THRESHOLD` の銘柄ペアのみエッジ化 |
| `FIRST_WEEK_ID` | `201501` | この週ID以降のみエッジを返す |
| `DEVICE` | `"cuda" if torch.cuda.is_available() else "cpu"` | 相関計算に使うデバイス |
...
### `residual_matrix_from_prices(prices_df, *, window_size_for_capm=52, residual_stored_path=..., use_cache=True) -> pd.DataFrame`
- **概要**: 全銘柄のCAPM残差リターンを計算し、`week_id x Code` 行列にピボットする。
- **処理内容**:
  1. `use_cache=True` かつキャッシュ（parquet）が存在すればロードのみ行い即return。
...
### `iter_firm_corr_edges(*, prices_df=None, residual_matrix=None, window_size_for_capm=52, window_size_for_corr=52, threshold=0.7, first_week_id=201501, residual_stored_path=..., use_cache=True, device=None, verbose=True) -> Iterator[Tuple[int, torch.Tensor, torch.Tensor, pd.Index]]`
- **概要**: 週IDごとにCAPM残差相関エッジを `yield` するジェネレータ（公開API）。
- **引数**: `prices_df` か `residual_matrix` のいずれかを渡す。両方渡した場合は `residual_matrix` を優先する。他のパラメータは `capm_corr_constants` のデフォルト値を継承。
- **処理内容**:
  ...
  3. `device` 未指定時は `capm_corr_constants.DEVICE` を使用。
...
パス・パラメータ定数を集約するモジュール。ロジックは持たない。
...
| `PATH_GRAPHINFO_DB` | `Template("./scripts/datap/graph_v2/DS/graph_info_${serial_id}.duckdb")`（`serial_id` でパラメータ化されたTemplate。`.safe_substitute(serial_id=...)` で実パスに解決する） |
...
| 定数 | 値 | 意味 |
|---|---|---|
| `START_WEEK_ID` | `200819` | 取得対象期間の開始週ID（`get_table_to_CAPM.sql` / `get_fin_data_by_week.sql` で `WHERE week_id > $START_WEEK_ID` に使用） |
...
- **クエリ**: `SELECT n.*, f.feats FROM node n JOIN feats f ON n.node_id = f.node_id WHERE n.node_type = '{node_type}'`（**f-stringによる直接埋め込み**。`node_type` はコード内部の固定文字列 `"stock"`/`"statement"` のみが渡される想定）。
...
- **前提**: `data_list` の列順が `edge` テーブルの列定義（`edge_id, edge_type, src_node_id, dst_node_id, edge_weight, observable_time_id`）と一致していること（`SELECT *` のため列名ではなく位置で対応付けられる）。
...
- **想定用途**: 将来的にCLI引数を受け取り、`dt_pipe.get_loaders(...)` や `dt_pipe.mock_code(...)` 等を呼び出すエントリポイントとして拡張される見込み。現時点で `python -m scripts.datap.graph_v2.main` を実行しても副作用は発生しない。
  ...
  └─ graphDataSet.__init__()
       ...
       └─ InMemoryDataset.__init__() [PyG標準]
            └─ process()  ※ processed_paths[0] が無い場合のみPyGが自動呼び出し
                 ...
                 └─ _build_full_graph()
                      ├─ fetch_node_table() ─ sql.fetch.node_table()
